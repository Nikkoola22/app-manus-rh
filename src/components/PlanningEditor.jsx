...
const PlanningEditor = ({ agentId, agentName, onSave, onCancel }) => {
      ...
      const planningsData = Object.entries(plannings).map(([jour, data]) => {
        if (data.plannings && data.plannings.length > 0) {
          return {
            ...
            pause_debut: data.plannings[0].pause_debut,
            pause_fin: data.plannings[0].pause_fin
          }
        }
        return null
      }).filter(Boolean)
...
const JourPlanningForm = ({ jour, jourLabel, planning, onSave, onCancel }) => {
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <Select
            ...
            onValueChange={(value) => setFormData(prev => ({ ...prev, heure_debut: value }))}
          >
        ...
        <div>
          ...
          <Select
            ...
            onValueChange={(value) => setFormData(prev => ({ ...prev, heure_fin: value }))}
          >
            <SelectTrigger className="w-full">
              <SelectValue placeholder="Sélectionner une heure" />
            </SelectTrigger>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <Select
            ...
            onValueChange={(value) => setFormData(prev => ({ ...prev, pause_debut: value }))}
          >
            <SelectTrigger className="w-full">
              <SelectValue placeholder="Aucune pause" />
            </SelectTrigger>
            <SelectContent className="bg-white border border-gray-300 shadow-lg z-50">
              <SelectItem value="" className="hover:bg-blue-50">Aucune pause</SelectItem>
              {heures.map(heure => (
                <SelectItem key={heure} value={heure} className="hover:bg-blue-50">
                  {heure}
                </SelectItem>
        ...
        <div>
          ...
          <Select
            ...
            onValueChange={(value) => setFormData(prev => ({ ...prev, pause_fin: value }))}
            disabled={!formData.pause_debut}
          >
            <SelectTrigger className="w-full" disabled={!formData.pause_debut}>
              <SelectValue placeholder="Aucune pause" />
            </SelectTrigger>
            <SelectContent className="bg-white border border-gray-300 shadow-lg z-50">
              <SelectItem value="" className="hover:bg-blue-50">Aucune pause</SelectItem>
